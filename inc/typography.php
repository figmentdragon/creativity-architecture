...
if ( ! function_exists( 'enqueue_google_fonts' ) ) {
	add_action( 'wp_enqueue_scripts', 'enqueue_google_fonts', 0 );
	/**
	...
	function enqueue_google_fonts() {
		...
		$not_google = str_replace( ' ', '+', typography_default_fonts() );
		...
		if ( ! empty( $font_settings ) ) {
			...
			foreach ( $font_settings as $key ) {
				...
				if ( strpos( $settings[$key], ':' ) !== false ) {
					$settings[$key] = current( explode( ':', $settings[$key] ) );
				}
				...
				$variants = get_google_font_variants( $settings[$key], $key );
		...
		$google_fonts = apply_filters( 'typography_google_fonts', $google_fonts );

		// Get the subset
		$subset = apply_filters( 'fonts_subset','' );
...
if ( ! function_exists( 'default_fonts_customize_register' ) ) {
	add_action( 'customize_register', 'default_fonts_customize_register' );
	/**
	...
	function default_fonts_customize_register( $wp_customize ) {
		...
		if ( method_exists( $wp_customize,'register_control_type' ) ) {
			$wp_customize->register_control_type( 'Typography_Customize_Control' );
		...
		$wp_customize->add_setting(
			...
			array(
				...
				'sanitize_callback' => 'sanitize_variants'
			)
		);
		...
		$wp_customize->add_control(
			new Typography_Customize_Control(
				$wp_customize,
				'body_typography',
				array(
					...
					'settings' => array(
						...
						'weight' => 'settings[body_font_weight]',
						'transform' => 'settings[body_font_transform]',
					),
				)
			)
		);
		...
		$wp_customize->add_setting(
			...
			array(
				...
				'type' => 'option',
				'sanitize_callback' => 'sanitize_decimal_integer',
				'transport' => 'postMessage'
			)
		);
...
if ( ! function_exists( 'get_all_google_fonts' ) ) {
	/**
	 ...
	 */
	function get_all_google_fonts( $amount = 'all' ) {
		...
		if ( apply_filters( 'alphabetize_google_fonts', true ) ) {
			asort( $fonts );
		}

		// Filter to allow us to modify the fonts array
		return apply_filters( 'google_fonts_array', $fonts );
	}
}

if ( ! function_exists( 'get_all_google_fonts_ajax' ) ) {
	add_action( 'wp_ajax_get_all_google_fonts_ajax', 'get_all_google_fonts_ajax' );
	/**
	 * Return an array of all of our Google Fonts.
	 *
	 *
	 */
	function get_all_google_fonts_ajax() {
		// Bail if the nonce doesn't check out
		if ( ! isset( $_POST[ 'customize_nonce' ] ) || ! wp_verify_nonce( sanitize_key( $_POST[ 'customize_nonce' ] ), 'customize_nonce' ) ) {
			wp_die();
		}

		// Do another nonce check
		check_ajax_referer( 'customize_nonce', 'customize_nonce' );
...
if ( ! function_exists( 'get_google_font_variants' ) ) {
	 ...
	 * Example: get_google_font_variation( 'Open Sans' )
	 *
	 */
	function get_google_font_variants( $font, $key = '' ) {
		// Don't need variants if we're using a system font
		if ( in_array( $font, typography_default_fonts() ) ) {
			return;
		}
		...
		if ( $variants ) {
			...
			return implode(',', apply_filters( 'typography_variants', $output ) );
		}
	}
}

if ( ! function_exists( 'get_google_font_category' ) ) {
	/**
	 * Wrapper function to find the category for chosen Google Font
	 * Example: get_google_font_category( 'Open Sans' )
	 *
	 *
	 *
	 ...
	 */
	function get_google_font_category( $font, $key = '' ) {
...
if ( ! function_exists( 'get_font_family_css' ) ) {
	/**
	 ...
	 */
	function get_font_family_css( $font, $settings, $default ) {
		$settings = wp_parse_args(
			get_option( $settings, array() ),
			$default
		);
		...
		$no_quotes = array(
			...
			apply_filters( 'typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' )
		);
		...
		if ( 'System Stack' == $font_family ) {
			$font_family = apply_filters( 'typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' );
		}
		...
		if ( in_array( $font_family, $no_quotes ) ) {
			...
			$wrapper_start = '"';
			$wrapper_end = '"' . get_google_font_category( $font_family, $font );
		}
...
if ( ! function_exists( 'add_to_font_customizer_list' ) ) {
	add_filter( 'typography_customize_list', 'add_to_font_customizer_list' );
	/**
	 ...
	 *
	 */
	function add_to_font_customizer_list( $fonts ) {
		...
		$select_fonts = get_all_google_fonts( apply_filters( 'number_of_fonts', 200 ) );
		...
		foreach ( $font_settings as $setting ) {
			...
			$id = strtolower( str_replace( ' ', '_', $settings[ $setting ] ) );

			if ( array_key_exists( $id, $select_fonts ) || in_array( $settings[ $setting ], typography_default_fonts() ) ) {
				continue;
			}

			$fonts[ strtolower( str_replace( ' ', '_', $settings[ $setting ] ) ) ] = array(
		...
		if ( function_exists( 'secondary_nav_get_defaults' ) ) {
			$secondary_nav_settings = wp_parse_args(
				get_option( 'secondary_nav_settings', array() ),
				secondary_nav_get_defaults()
			);

			$secondary_nav_id = strtolower( str_replace( ' ', '_', $secondary_nav_settings[ 'font_secondary_navigation' ] ) );

			if ( ! array_key_exists( $secondary_nav_id, $select_fonts ) && ! in_array( $secondary_nav_settings[ 'font_secondary_navigation' ], typography_default_fonts() ) ) {
