 ...
 */
function local_gravatars( $avatar ) {

	if ( true !== apply_filters( 'local_gravatars', false ) ) {
		return $avatar;
	}
	...
	if ( isset( $srcset[1] ) && isset( $srcset[1][0] ) ) {
		...
		$local_gravatar = get_local_gravatar( $url );
...
add_filter( 'get_avatar', 'local_gravatars' );
 ...
 */
function get_local_gravatar( $url ) {
	...
	$pbf_dir    = $upload_dir['basedir'] . '/THEMENAME/';
	$avatar_dir = $upload_dir['basedir'] . '/THEMENAME/gravatar/';
	$avatar_url = $upload_dir['baseurl'] . '/THEMENAME/gravatar/';
