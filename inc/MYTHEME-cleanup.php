...
function MYTHEME_scripts_and_styles() {
  ...
  if (!is_admin()) {
		...
		wp_register_script( 'MYTHEME-modernizr', get_stylesheet_directory_uri() . '/library/js/libs/modernizr.custom.min.js', array(), '2.5.3', false );

		// register main stylesheet
		wp_register_style( 'MYTHEME-stylesheet', get_stylesheet_directory_uri() . '/library/css/style.css', array(), '', 'all' );

		// ie-only style sheet
		wp_register_style( 'MYTHEME-ie-only', get_stylesheet_directory_uri() . '/library/css/ie.css', array(), '' );
    ...
    if ( is_singular() AND comments_open() AND (get_option('thread_comments') == 1)) {
		...
		wp_register_script( 'MYTHEME-js', get_stylesheet_directory_uri() . '/library/js/scripts.js', array( 'jquery' ), '', true );

		// enqueue styles and scripts
		wp_enqueue_script( 'MYTHEME-modernizr' );
		wp_enqueue_style( 'MYTHEME-stylesheet' );
		wp_enqueue_style( 'MYTHEME-ie-only' );

		$wp_styles->add_data( 'MYTHEME-ie-only', 'conditional', 'lt IE 9' ); // add conditional wrapper around ie stylesheet
...
function MYTHEME_related_posts() {
	...
	if($tags) {
		...
		if($related_posts) {
			...
			<?php echo '<li class="no_related_post">' . __( 'No Related Posts Yet!', 'MYTHEME' ) . '</li>'; ?>
		<?php }
	}
	wp_reset_postdata();
	echo '</ul>';
