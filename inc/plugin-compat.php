...
function setup_woocommerce() {
	...
	add_action( 'woocommerce_sidebar', 'construct_sidebars' );
}

if ( ! function_exists( 'woocommerce_start' ) ) {
	add_action( 'woocommerce_before_main_content', 'woocommerce_start', 10 );
	/**
	 * Add WooCommerce starting wrappers
	 *
	 */
	function woocommerce_start() { ?>
		<div id="primary" <?php content_class();?>>
			<main id="main" <?php main_class(); ?>>
				<?php
				/**
				 * before_main_content hook.
				 *
				 */
				do_action( 'before_main_content' );
				?>
				<article id="post-<?php the_ID(); ?>" <?php post_class(); ?> <?php article_schema( 'CreativeWork' ); ?>>
					<div class="inside-article">
						 ...
						 *
						 *
						 * @hooked featured_page_header_inside_single - 10
						 */
...
if ( ! function_exists( 'woocommerce_end' ) ) {
	add_action( 'woocommerce_after_main_content', 'woocommerce_end', 10 );
	/**
	 * Add WooCommerce ending wrappers
	 *
	 */
	function woocommerce_end() { ?>
						</div><!-- .entry-content -->
						<?php
						/**
						 * after_content hook.
...
if ( ! function_exists( 'woocommerce_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'woocommerce_css', 100 );
	/**
	 * Add WooCommerce CSS
	 *
	 */
	function woocommerce_css() {
		...
		$mobile = apply_filters( 'mobile_media_query', '(max-width:768px)' );
...
if ( ! function_exists( 'bbpress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'bbpress_css', 100 );
	/**
	 * Add bbPress CSS
	 *
	 */
	function bbpress_css() {
...
if ( ! function_exists( 'buddypress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'buddypress_css', 100 );
	/**
	 * Add BuddyPress CSS
	 *
	 */
	function buddypress_css() {
...
if ( ! function_exists( 'beaver_builder_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'beaver_builder_css', 100 );
	/**
