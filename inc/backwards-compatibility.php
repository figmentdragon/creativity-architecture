 ...
 *
 * Switching to Kirki 4 means is_plugin_active is no longer present in kirki.php in the Premium Add-On.
 ...
 */
function kirki_is_plugin_active_fatal_error() {

	// Safety first. Check if is_premium exists.
	...
	// Stop if PREMIUM_VERSION is not defined.
	if ( ! defined( 'PREMIUM_VERSION' ) ) {
		return;
	}

	// Stop if PREMIUM_VERSION is above 2.7.6.2.
	if ( version_compare( PREMIUM_VERSION, '2.7.6.2', '>' ) ) {
		return;
	}
...
add_action( 'after_setup_theme', 'kirki_is_plugin_active_fatal_error', 5 );
