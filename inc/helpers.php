 ...
 */
function archive_schema_markup() {

	// Default itemtype.
	$itemtype = 'CreativeWork';

	// Make result filterable.
	$result = apply_filters( 'archive_itemtype', $itemtype );
...
function single_schema_markup() {
	...
	// Make result filterable.
	$result = apply_filters( 'single_itemtype', $itemtype );
		...
		<li id="comment-<?php comment_ID(); ?>">
			...
			<article <?php comment_class(); ?> itemscope="itemscope" itemtype="https://schema.org/Comment">
				...
				<footer class="comment-meta">
					...
					<div class="comment-author-info">
						...
						if ( svg_enabled() ) {
							...
							$date_icon = '<i class="themenamef themenamef-clock"></i>';
						}
						?>

						<time class="comment-time" datetime="<?php comment_time( 'c' ); ?>" itemprop="datePublished">

							<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>"><?php echo $date_icon; ?> <?php comment_time( __( 'F d, Y', 'THEMENAME' ) ); ?></a>
				...
				<div class="notice notice-warning">
					<?php _e( 'Your comment has yet to be approved.', 'THEMENAME' ); ?>
				</div>
...
if ( ! function_exists( 'svg_enabled' ) ) {

	/**
	 * Helper function to check if SVG's are enabled.
	 */
	function svg_enabled() {

		if ( apply_filters( 'svg', false ) ) {
			return true;
		}
...
if ( ! function_exists( 'svg' ) ) {
	...
	function svg( $icon ) {
		...
		if ( 'twitter' === $icon ) {
			$output = '<svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32" role="img" aria-hidden="true">
							<path d="M10.086 28.664c12.027 0 18.604-9.964 18.604-18.604 0-0.283-0.006-0.565-0.019-0.845 1.277-0.923 2.386-2.074 3.262-3.385-1.172 0.52-2.432 0.871-3.755 1.029 1.35-0.809 2.386-2.090 2.875-3.617-1.264 0.749-2.662 1.294-4.152 1.587-1.193-1.271-2.892-2.066-4.773-2.066-3.611 0-6.54 2.928-6.54 6.538 0 0.513 0.057 1.012 0.17 1.491-5.434-0.273-10.253-2.875-13.477-6.831-0.562 0.966-0.886 2.089-0.886 3.286 0 2.269 1.154 4.271 2.91 5.443-1.072-0.033-2.080-0.328-2.961-0.818-0.001 0.027-0.001 0.055-0.001 0.083 0 3.167 2.254 5.811 5.246 6.41-0.549 0.149-1.127 0.23-1.724 0.23-0.421 0-0.831-0.042-1.229-0.118 0.832 2.598 3.246 4.488 6.108 4.541-2.238 1.754-5.057 2.799-8.121 2.799-0.528 0-1.048-0.030-1.56-0.090 2.894 1.854 6.33 2.937 10.023 2.937z"></path>
						</svg>';
		}
 ...
 */
function navigation_classes() {
	echo apply_filters( 'navigation_classes', 'navigation' );
}
 ...
 */
function inner_content( $echo = true ) {
	...
	if ( is_singular() ) {
		...
		$inner_content = $fullwidth ? '' : apply_filters( 'inner_content', '<div id="inner-content" class="container container-center padding-medium"' . $custom_width . '>' );

		// Check if Premium Add-On is active and template is not set to contained.
		if ( is_premium() && ! $contained && ! $custom_width ) {
			...
			$fullwidth_global = isset( $settings['fullwidth_global'] ) ? $settings['fullwidth_global'] : array();
		...
		// On archives, we only add the inner_content filter.
		$inner_content = apply_filters( 'inner_content', '<div id="inner-content" class="container container-center padding-medium">' );
...
function title() {
	...
	if ( is_premium() ) {
		...
		$removetitle_global = isset( $settings['removetitle_global'] ) ? $settings['removetitle_global'] : array();
...
function remove_footer() {
	...
	if ( $remove_footer ) {
		...
		remove_action( 'before_footer', 'custom_footer' );
	}

}
add_action( 'wp', 'remove_footer' );
...
function scrolltop() {
	...
	if ( get_theme_mod( 'layout_scrolltop' ) ) {
		...
		if ( svg_enabled() ) {
			...
			$icon = '<i class="themenamef themenamef-arrow-up"></i>';
		}

		echo '<a class="scrolltop" rel="nofollow" href="javascript:void(0)" data-scrolltop-value="' . (int) $scrollTop . '">';
		echo '<span class="screen-reader-text">' . __( 'Scroll to Top', 'THEMENAME' ) . '</span>';
		echo $icon;
		echo '</a>';
...
add_action( 'wp_footer', 'scrolltop' );
 ...
 * archive-content
 * + {post-type}-archive
 * + {archive-type}-content (for post archives)
 *
 * + {post-type}-archive-content (for cpt archives)
 * + {post-type}-taxonomy-content (for cpt-related taxonomies)
 *
...
function archive_class() {
	...
	if ( is_date() ) {
		$archive_class = ' archive-content post-archive date-content';
	} elseif ( is_category() ) {
		$archive_class = ' archive-content post-archive category-content';
	} elseif ( is_tag() ) {
		$archive_class = ' archive-content post-archive tag-content';
	} elseif ( is_author() ) {
		$archive_class = ' archive-content post-archive author-content';
	} elseif ( is_home() ) {
		$archive_class = ' archive-content post-archive blog-content';
	} elseif ( is_search() ) {
		$archive_class = ' archive-content post-archive search-content';
		...
		$archive_class  = ' archive-content';
		$archive_class .= ' ' . $post_type . '-archive';
		$archive_class .= ' ' . $post_type . '-archive-content';
		...
		$archive_class .= ' ' . $post_type . '-archive';
		$archive_class .= ' ' . $post_type . '-archive-content';
		$archive_class .= ' ' . $post_type . '-taxonomy-content';

	}

	return apply_filters( 'archive_class', $archive_class );
 ...
 */
function singular_class() {

	if ( is_singular( 'post' ) ) {
		$singular_class = ' single-content post-content';
	} elseif ( is_attachment() ) {
		$singular_class = ' single-content attachment-content';
	} elseif ( is_page() ) {
		$singular_class = ' single-content page-content';
	} elseif ( is_404() ) {
		$singular_class = ' single-content 404-content';
	} else {
		$post_type      = get_post_type();
		$singular_class = ' single-content ' . $post_type . '-content';
	}

	return apply_filters( 'singular_class', $singular_class );
...
function archive_header() {
	...
	if ( is_author() ) {
		...
		$blog_title = apply_filters( 'blog_page_title', '' );
...
add_filter( 'get_the_archive_title', 'archive_title', 10 );
 ...
 */
function do_post_links() {

	if ( 'hide' === get_theme_mod( 'single_post_nav' ) ) {
		return;
	}

	do_action( 'before_post_links' );

	?>

	<nav class="post-links clearfix" aria-label="<?php _e( 'Post Navigation', 'THEMENAME' ); ?>">

		<span class="screen-reader-text"><?php _e( 'Post Navigation', 'THEMENAME' ); ?></span>

		<?php
		previous_post_link( '<span class="previous-post-link">%link</span>', apply_filters( 'previous_post_link', __( '&larr; Previous Post', 'THEMENAME' ) ) );
		next_post_link( '<span class="next-post-link">%link</span>', apply_filters( 'next_post_link', __( 'Next Post &rarr;', 'THEMENAME' ) ) );
		?>
	...
	do_action( 'after_post_links' );

}
add_action( 'post_links', 'do_post_links' );
 ...
 */
function do_posts_pagination() {

	the_posts_pagination(
		array(
			'mid_size'  => apply_filters( 'posts_pagination_size', 2 ),
			'prev_text' => apply_filters( 'posts_navigation_prev_text', __( '&larr; Previous', 'THEMENAME' ) ),
			'next_text' => apply_filters( 'posts_navigation_next_text', __( 'Next &rarr;', 'THEMENAME' ) ),
		)
	);

}
add_action( 'posts_pagination', 'do_posts_pagination' );

if ( ! function_exists( 'has_responsive_breakpoints' ) ) {
	 ...
	 * @return boolean.
	 */
	function has_responsive_breakpoints() {
		...
		if ( ! empty( $settings['breakpoint_medium'] ) || ! empty( $settings['breakpoint_desktop'] ) || ! empty( $settings['breakpoint_mobile'] ) ) {
			return true;
		} else {
			return false;
		}
...
add_action( 'sidebar_right', 'do_sidebar_right' );

/**
 * Render left sidebar.
 */
function do_sidebar_left() {
...
add_action( 'sidebar_left', 'do_sidebar_left' );
...
function sidebar_layout() {
	...
	return apply_filters( 'sidebar_layout', $sidebar );
...
function article_meta() {
	...
	if ( is_array( $blog_meta ) && ! empty( $blog_meta ) ) {
		...
		do_action( 'before_article_meta' );
		echo '<p class="article-meta">';
		do_action( 'article_meta_open' );

		foreach ( $blog_meta as $value ) {

			switch ( $value ) {
				case 'author':
					do_action( 'before_author_meta' );
					do_action( 'author_meta' );
					do_action( 'after_author_meta' );
					break;
				case 'date':
					do_action( 'before_date_meta' );
					do_action( 'date_meta' );
					do_action( 'after_date_meta' );
					break;
				case 'comments':
					do_action( 'before_comments_meta' );
					do_action( 'comments_meta' );
					do_action( 'after_comments_meta' );
					break;
				default:
					break;
			}
		...
		do_action( 'article_meta_close' );
		echo '</p>';
		do_action( 'after_article_meta' );
 ...
 * Article meta (author).
 */
function do_author_meta() {

	$rtl         = is_rtl();
	$avatar      = get_theme_mod( 'blog_author_avatar' );
	$avatar_size = apply_filters( 'author_meta_avatar_size', 128 );
...
add_action( 'author_meta', 'do_author_meta' );
...
function do_date_meta() {
	...
	echo '<span class="posted-on">' . __( 'Posted on', 'THEMENAME' ) . '</span> <time class="article-time published" datetime="' . get_the_date( 'c' ) . '" itemprop="datePublished">' . get_the_date() . '</time>';
	echo '<span class="article-meta-separator">' . apply_filters( 'article_meta_separator', ' | ' ) . '</span>';

}
add_action( 'date_meta', 'do_date_meta' );

/**
 * Article meta (comments).
 */
function do_comments_meta() {

	echo '<span class="comments-count">';

	comments_number(
		__( '<span>No</span> Comments', 'THEMENAME' ),
		__( '<span>1</span> Comment', 'THEMENAME' ),
		__( '<span>%</span> Comments', 'THEMENAME' )
	);
...
add_action( 'comments_meta', 'do_comments_meta' );
	...
	if ( $custom_menu ) {
		...
		wp_nav_menu(
			array(
				'theme_location' => 'main_menu',
				'container'      => false,
				'menu_class'     => 'menu sub-menu' . sub_menu_alignment() . sub_menu_animation() . menu_hover_effect(),
				'depth'          => 4,
				'fallback_cb'    => 'main_menu_fallback',
			)
		);
...
add_action( 'mobile_menu', 'mobile_nav_menu' );
 ...
 */
function menu() {
	get_template_part( 'inc/template-parts/navigation/' . apply_filters( 'menu_variation', get_theme_mod( 'menu_position', 'menu-right' ) ) );
}
...
function mobile_menu() {
	get_template_part( 'inc/template-parts/navigation/' . apply_filters( 'mobile_menu_variation', get_theme_mod( 'mobile_menu_options', 'menu-mobile-hamburger' ) ) );
}
...
function sub_menu_indicators( $title, $item, $args, $depth ) {
	...
	if ( isset( $item->classes ) && in_array( 'menu-item-has-children', $item->classes ) ) {
		...
		if ( svg_enabled() ) {
			...
			$title .= ' <i class="themenamef themenamef-arrow-down" aria-hidden="true"></i>';
		}
		...
		if ( isset( $item->classes ) && in_array( 'menu-item-has-children', $item->classes ) ) {
			...
			if ( svg_enabled() ) {
				...
				'<button class="submenu-toggle" aria-expanded="false">
					<span class="screen-reader-text">' . __( 'Menu Toggle', 'THEMENAME' ) . '</span>
					' . svg( 'arrow-down' ) . svg( 'arrow-up' ) . '
				</button>';

			} else {

				$item_output .= '<button class="submenu-toggle" aria-expanded="false"><span class="screen-reader-text">' . __( 'Menu Toggle', 'THEMENAME' ) . '</span><i class="themenamef themenamef-arrow-down" aria-hidden="true"></i></button>';
...
function logo_attributes() {
	...
	return apply_filters( 'logo_attributes', $attributes );
 ...
 * @param int $id the ID.
 */
function page_builder_compatibility( $id ) {
	...
	if ( $fl_enabled || 'builder' === $elementor ) {
		...
		update_post_meta( $id, 'options', $mydata );

	}

}
// add_action( 'page_builder_compatibility', 'task' );
