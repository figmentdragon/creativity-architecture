...
function get_svg( $args = array() ) {
	...
	if ( empty( $args ) ) {
		return __( 'Please define default parameters in the form of an array.', '__THEMENAE__' );
	}

	// Define an icon.
	if ( false === array_key_exists( 'icon', $args ) ) {
		return __( 'Please define an SVG icon filename.', '__THEMENAE__' );
	}
...
function nav_menu_social_icons( $item_output, $item, $depth, $args ) {
	...
	if ( 'social-menu' === $args->theme_location || 'social-links-menu' === $args->menu_class ) {
		...
		foreach ( $social_icons as $attr => $value ) {
			if ( false !== strpos( $item_output, $attr ) ) {
				$item_output = str_replace( $args->link_before, get_svg( array( 'icon' => esc_attr( $value ) ) ) . '<span>', $item_output );
			}
		}
	}
