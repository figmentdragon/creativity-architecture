...
/**
 * Shows a breadcrumb for all types of pages.  This is a wrapper function for the Breadcrumbs class,
 * which should be used in theme templates.
 *
 * @since  0.1.0
 * @access public
 * @param  array $args Arguments to pass to Breadcrumbs.
 * @return void
 */
function do_breadcrumbs( $args = array() ) {
	...
	if ( ! is_object( $breadcrumb ) )
		$breadcrumb = new Breadcrumbs( $args );
...
add_action( 'inner_content_open', 'breadcrumbs_content' );

/**
 * Add breadcrumbs below header.
 */
function breadcrumbs_header() {
...
add_action( 'after_header', 'breadcrumbs_header' );
...
class Breadcrumbs {
	...
	public function __construct( $args = array() ) {
		...
		$defaults = array(
			...
			'list_tag'        => 'ul',
			'item_tag'        => 'li',
			'separator'		  => apply_filters( 'breadcrumbs_separator', '/' ),
			'show_on_front'   => true,
			'network'         => false,
			'show_title'      => true,
			'labels'          => array(),
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			$breadcrumb .= sprintf(
				'<%s class="breadcrumbs" itemscope="itemscope" itemtype="https://schema.org/BreadcrumbList">',
			...
			$breadcrumb = sprintf(
				'<%1$s aria-label="%2$s" class="breadcrumbs-wrapper" itemprop="breadcrumb">%3$s%4$s%5$s</%1$s>',
		...
		$breadcrumb = apply_filters( 'breadcrumbs', $breadcrumb, $this->args );
		...
		if( 'header' === get_theme_mod( 'breadcrumbs_position' ) ) {
			...
			$output .= '<div class="container container-center">';
			$output .= $breadcrumb;
			$output .= '</div>';
			$output .= '</div>';
	...
	protected function set_labels() {
		...
		$defaults = array(
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', '__THEMENAE__' ),
			'home'                => esc_html__( 'Home', '__THEMENAE__' ),
			'error_404'           => esc_html__( '404 Not Found', '__THEMENAE__' ),
			'archives'            => esc_html__( 'Archives', '__THEMENAE__' ),
			// Translators: %s is the search query.
			'search'              => esc_html__( 'Search results for: %s', '__THEMENAE__' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s', '__THEMENAE__' ),
			// Translators: %s is the page number.
			'paged_comments'      => esc_html__( 'Comment Page %s', '__THEMENAE__' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s', '__THEMENAE__' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s', '__THEMENAE__' ),
