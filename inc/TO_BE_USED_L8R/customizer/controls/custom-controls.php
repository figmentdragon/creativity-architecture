		...
		public function render_content() {
			$dropdown = wp_dropdown_categories(
				array(
					...
					'hide_if_empty'    => false,
					'show_option_all'  => esc_html__( 'All Categories', '__THEMENAE__' ),
				)
			);
			...
			echo '<p class="description">' . esc_html__( 'Hold down the Ctrl (windows) / Command (Mac) button to select multiple options.', '__THEMENAE__' ) . '</p>';
		}
	}
	...
	class Multi_Cat_Project_Type extends WP_Customize_Control {
		...
		public function render_content() {
			...
			if ( ! taxonomy_exists( $taxonomy ) ) {
				echo '<p class="description">' . esc_html__( 'CPT Project Type does not exist. Make sure Essential Content Types plugin is activated with Portfolio CPT enabled.', '__THEMENAE__' ) . '</p>';
			...
			<ul class="custom-sortable">
				...
				foreach ( $sortable_sections as $key => $value ) {
					...
					if ( isset( $value['section'] ) ) {
						echo '<a href="javascript:wp.customize.section( \'' . esc_attr( $value['section'] ) . '\' ).focus();">' . esc_html__( 'Edit', '__THEMENAE__' ) . '</a>';
					}
					echo '</li>';
				}
			    ?>
