...
function sort_sections_list( $wp_customize ) {
	foreach ( $wp_customize->sections() as $section_key => $section_object ) {
		if ( false !== strpos( $section_key, '__THEMENAE__' ) && 'important_links' !== $section_key ) {
			$options[] = $section_key;
		}
	}
...
function section_visibility_options() {
	$options = array(
		...
		'disabled'    => esc_html__( 'Disabled', '__THEMENAE__' ),
	);
...
function sections_layout_options() {
	$options = array(
		'layout-one'   => esc_html__( '1 column', '__THEMENAE__' ),
		'layout-two'   => esc_html__( '2 columns', '__THEMENAE__' ),
		'layout-three' => esc_html__( '3 columns', '__THEMENAE__' ),
		'layout-four'  => esc_html__( '4 columns', '__THEMENAE__' ),
	);
...
function section_type_options() {
	$options = array(
		'post'     => esc_html__( 'Post', '__THEMENAE__' ),
		'page'     => esc_html__( 'Page', '__THEMENAE__' ),
		'category' => esc_html__( 'Category', '__THEMENAE__' ),
		'custom'   => esc_html__( 'Custom', '__THEMENAE__' ),
	);
...
function get_pagination_types() {
	$pagination_types = array(
		'default' => esc_html__( 'Default(Older Posts/Newer Posts)', '__THEMENAE__' ),
		'numeric' => esc_html__( 'Numeric', '__THEMENAE__' ),
	);
...
function generate_post_array( $post_type = 'post' ) {
	...
	$output['0']= esc_html__( '-- Select --', '__THEMENAE__' );

	foreach ( $posts as $post ) {
		$output[ $post->ID ] = ! empty( $post->post_title ) ? $post->post_title : sprintf( __( '#%d (no title)', '__THEMENAE__' ), $post->ID );
	}
...
function content_show() {
	$options = array(
		'excerpt'      => esc_html__( 'Show Excerpt', '__THEMENAE__' ),
		'full-content' => esc_html__( 'Full Content', '__THEMENAE__' ),
		'hide-content' => esc_html__( 'Hide Content', '__THEMENAE__' ),
	);
...
function meta_show() {
	$options = array(
		'show-meta' => esc_html__( 'Show Meta', '__THEMENAE__' ),
		'hide-meta' => esc_html__( 'Hide Meta', '__THEMENAE__' ),
	);
