...
function main_menu_fallback() {
	...
	if ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) {
		...
		<ul class="menu">
			<li class="menu-item">
				<a href="javascript:void(0)" onclick="parent.wp.customize.panel( 'nav_menus' ).focus()"><?php _e( 'Add Menu', '__THEMENAE__' ); ?></a>
			</li>
		</ul>
		...
		<ul class="menu">
			<li class="menu-item">
				<a href="<?php echo esc_url( admin_url( '/nav-menus.php' ) ); ?>"><?php _e( 'Add Menu', '__THEMENAE__' ); ?></a>
			</li>
		</ul>
...
function menu_fallback() {
	...
	if ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) {
		...
		echo '<a href="' . esc_url( admin_url( '/nav-menus.php' ) ) . '">' . __( 'Add Menu', '__THEMENAE__' ) . '</a>';
 ...
 */
function menu_description_html( $menu_item ) {
...
add_filter( 'wp_setup_nav_menu_item', 'menu_description_html' );

/**
 * Remove "Background" customizer link from Appearance menu.
 */
function remove_background_link() {
...
add_action( 'admin_menu', 'remove_background_link', 999 );
