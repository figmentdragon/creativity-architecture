...
function posted_on() {
		...
		$posted_on = '<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . esc_html__( 'Posted on ', '__THEMENAE__' ) . $time_string . '</a>';
...
function by_line() {
		...
		echo '<span class="byline">' .  esc_html__( 'Posted By ', '__THEMENAE__' ) . $byline . '</span>';
	}
function cat_list() {
		// Hide category and tag text for pages.
		if ( 'post' === get_post_type() ) {
			/* translators: used between list items, there is a space after the / */
			$categories_list = get_the_category_list( esc_html__( ', ', '__THEMENAE__' ) );
			if ( $categories_list ) {
				/* translators: 1: list of categories. */
				printf( '<span class="cat-links"><span class="screen-reader-text">%1$s </span>%2$s</span>', esc_html__(  'Cat Links', '__THEMENAE__' ), $categories_list );
			}
		} elseif ( 'jetpack-portfolio' == get_post_type() ) {
			/* translators: used between list items, there is a space after the / */
			$categories_list = get_the_term_list( get_the_ID(), 'jetpack-portfolio-type', '', esc_html__( ' / ', '__THEMENAE__' ) );
...
function entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list  ) {
				echo '<span class="cat-links">' . get_svg( array( 'icon' => 'folder-open' ) ) . '<span class="screen-reader-text">' . __( 'Categories', '__THEMENAE__' ) . '</span>' . $categories_list . '</span>';
			}

			$tags_list = get_the_tag_list( '', ' ' ); // Separate list by space.

			if ( $tags_list  ) {
				echo '<span class="tags-links">' . get_svg( array( 'icon' => 'tag' ) ) . '<span class="screen-reader-text">' . __( 'Tags', '__THEMENAE__' ) . ',' . '</span>' . $tags_list . '</span>';
			}
		}
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', '__THEMENAE__' ),
						array(
							'span' => array(
								'class' => array(),
							),
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', '__THEMENAE__' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
function entry_meta_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list && categorized_blog() ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', '__THEMENAE__' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html__( ', ', '__THEMENAE__' ) );
			if ( $tags_list ) {
				/* translators: %1$s : tag list */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', '__THEMENAE__' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
		...
		if ( is_sticky() ) {
				echo '<p>' . esc_html_x( 'Featured post', 'Label for sticky posts', '__THEMENAE__' ) . '</p>';
			}
			...
			if ( has_category() || has_tag() ) {
				...
				/* translators: Used between list items, there is a space after the comma. */
				$categories_list = get_the_category_list( __( ', ', '__THEMENAE__' ) );
				if ( $categories_list ) {
					printf(
						/* translators: %s: List of categories. */
						'<span class="cat-links">' . esc_html__( 'Categorized as %s', '__THEMENAE__' ) . ' </span>',
						$categories_list // phpcs:ignore WordPress.Security.EscapeOutput
					);
				}

				/* translators: Used between list items, there is a space after the comma. */
				$tags_list = get_the_tag_list( '', __( ', ', '__THEMENAE__' ) );
				if ( $tags_list ) {
					printf(
						/* translators: %s: List of tags. */
						'<span class="tags-links">' . esc_html__( 'Tagged %s', '__THEMENAE__' ) . '</span>',
...
function entry_date_author() {
	...
	$meta .= sprintf( '<span class="posted-on screen-reader-text">%3$s' . '<span class="date-label screen-reader-text">%1$s</span><a href="%2$s" rel="bookmark">%4$s</a></span>',
		_x( 'Posted on', 'Used before publish date.', '__THEMENAE__' ),
		esc_url( get_permalink() ),
		esc_html__( 'Posted on ', '__THEMENAE__' ),
		$time_string
	);
	...
	$byline = sprintf(
		...
		__( '<span class="author-label screen-reader-text">By </span>%s', '__THEMENAE__' ),
		'<span class="author vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . get_the_author() . '</a></span>'
	);

	$meta .= sprintf( '<span class="byline">%1$s%2$s</span>',
		esc_html__( ' By ', '__THEMENAE__' ),
		$byline
	 );
...
function entry_category_date() {
		...
		$portfolio_categories_list = get_the_term_list( get_the_ID(), 'jetpack-portfolio-type', '<span class="portfolio-entry-meta entry-meta">', esc_html_x( ', ', 'Used between list items, there is a space after the comma.', '__THEMENAE__' ), '</span>' );

		if ( 'jetpack-portfolio' === get_post_type() ) {
			$meta .= sprintf( '<span class="cat-links">%1$s%2$s</span>',
				sprintf( _x( '<span class="screen-reader-text">Categories: </span>', 'Used before category names.', '__THEMENAE__' ) ),
		...
		$meta .= sprintf( '<span class="posted-on">%1$s<a href="%2$s" rel="bookmark">%3$s</a></span>',
			sprintf( __( '<span class="date-label">Posted on </span>', '__THEMENAE__' ) ),
			esc_url( get_permalink() ),
			$time_string
		);
...
function footer_content() {
	...
	$footer_content = sprintf( _x( 'Copyright &copy; %1$s %2$s %3$s', '1: Year, 2: Site Title with home URL, 3: Privacy Policy Link', '__THEMENAE__' ), '[the-year]', '[site-link]', '[privacy-policy-link]' ) . '<span class="sep"> | </span>' . esc_html( $theme_data->get( 'Name' ) ) . '&nbsp;' . esc_html__( 'by', '__THEMENAE__' ) . '&nbsp;<a target="_blank" href="' . esc_url( $theme_data->get( 'AuthorURI' ) ) . '">' . esc_html( $theme_data->get( 'Author' ) ) . '</a>';
	...
	$replace = array( esc_attr( date_i18n( __( 'Y', '__THEMENAE__' ) ) ), '<a href="'. esc_url( home_url( '/' ) ) .'">'. esc_attr( get_bloginfo( 'name', 'display' ) ) . '</a>', function_exists( 'get_the_privacy_policy_link' ) ? get_the_privacy_policy_link() : '' );
...
if ( ! function_exists( 'the_posts_navigation' ) ) {
	...
	function the_posts_navigation() {
		the_posts_pagination(
			array(
				'before_page_number' => esc_html__( 'Page', '__THEMENAE__' ) . ' ',
				'mid_size'           => 0,
				'prev_text'          => sprintf(
					'%s <span class="nav-prev-text">%s</span>',
					is_rtl() ? get_icon_svg( 'ui', 'arrow_right' ) : get_icon_svg( 'ui', 'arrow_left' ),
					wp_kses(
						__( 'Newer <span class="nav-short">posts</span>', '__THEMENAE__' ),
						array(
							'span' => array(
								'class' => array(),
							),
				...
				'next_text'          => sprintf(
					...
					wp_kses(
						__( 'Older <span class="nav-short">posts</span>', '__THEMENAE__' ),
						array(
							'span' => array(
								'class' => array(),
							),
...
function slider_entry_category() {
	...
	if ( 'jetpack-portfolio' === get_post_type( ) ) {
		$meta .= sprintf( '<span class="cat-links">' .'<span class="cat-label screen-reader-text">%1$s</span>%2$s</span>',
			sprintf( _x( 'Categories', 'Used before category names.', '__THEMENAE__' ) ),
			$portfolio_categories_list
		);
	}
...
function multipage_navigation() {
		...
		'before' => '<div class="page-links">' . esc_html__( 'Pages:', '__THEMENAE__' ),
...
function blog_navigation() {
		...
		the_posts_pagination( array(
			...
			'prev_text' => '<span class="nav-arrow">&laquo</span><span class="screen-reader-text">' . esc_html_x( 'Previous Posts', 'pagination', '__THEMENAE__' ) . '</span>',

			'next_text' => '<span class="screen-reader-text">' . esc_html_x( 'Next Posts', 'pagination', '__THEMENAE__' ) . '</span><span class="nav-arrow">&raquo;</span>',
        ...
        <div class="media-body">
            ...
            <div class="comment-content" itemprop="commentText">
                ...
                <p><em class="awaiting">
                        ...
                        <?php esc_html_e( 'Your comment is awaiting moderation.', '__THEMENAE__' ) ?></em></p>
            ...
            <div class="comment-reply">
                ...
                <?php edit_comment_link( __( 'Edit', '__THEMENAE__'), ' &middot; ', '' ) ?>
...
if ( ! function_exists( 'categories' ) ) :
	function categories() {
		echo '<p id="post-categories">', esc_html_e( 'Categories: ', '__THEMENAE__' ) .  the_category(' &bull; ') .'</p>';
	}
endif;

function entry_tags() {
	echo get_the_tag_list( sprintf( // WPCS: XSS OK.
		'<span>%s: ', __( 'Tags: ', '__THEMENAE__' ) ), ' &bull; ', '</span>' );
	}

function comment_count() {
	if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<li class="comments-link">';
		comments_popup_link( sprintf( __( 'Leave a comment<span class="screen-reader-text"> on %s</span>', '__THEMENAE__' ), get_the_title() ) );
		echo '</li>';
	}
} endif;
...
function sticky_entry_post() {
	if ( is_sticky() && ! is_paged() ) {
		echo '<div class="featured-label">', esc_html_e('Featured', '__THEMENAE__'), '</div>';
	}
} endif;
 ...
 */
function posted_on() {
    printf( __( 'Posted on <a href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s" pubdate>%4$s</time></a><span class="byline"> by <span class="author vcard"><a class="url fn n" href="%5$s" title="%6$s" rel="author">%7$s</a></span></span>', '__THEMENAE__' ),
        esc_url( get_permalink() ),
...
function content_nav( $nav_id ) {
    ...
    <nav role="navigation" id="<?php echo $nav_id; ?>" class="<?php echo $nav_class; ?>">
        <h1 class="assistive-text"><?php _e( 'Post navigation', '__THEMENAE__' ); ?></h1>

    <?php if ( is_single() ) : // navigation links for single posts ?>

        <?php previous_post_link( '<div class="nav-previous">%link</div>', '<span class="meta-nav">' . _x( '&larr;', 'Previous post link', '__THEMENAE__' ) . '</span> %title' ); ?>
        <?php next_post_link( '<div class="nav-next">%link</div>', '%title <span class="meta-nav">' . _x( '&rarr;', 'Next post link', '__THEMENAE__' ) . '</span>' ); ?>

    <?php elseif ( $wp_query->max_num_pages > 1 && ( is_home() || is_archive() || is_search() ) ) : // navigation links for home, archive, and search pages ?>

        <?php if ( get_next_posts_link() ) : ?>
        <div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Older posts', '__THEMENAE__' ) ); ?></div>
        <?php endif; ?>

        <?php if ( get_previous_posts_link() ) : ?>
        <div class="nav-next"><?php previous_posts_link( __( 'Newer posts <span class="meta-nav">&rarr;</span>', '__THEMENAE__' ) ); ?></div>
        <?php endif; ?>
