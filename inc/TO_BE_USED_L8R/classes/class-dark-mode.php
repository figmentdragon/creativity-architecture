...
class Dark_Mode {
	...
	public function __construct() {
		...
		// Add HTML classes.
		add_filter( 'html_classes', array( $this, 'html_classes' ) );
	...
	public function editor_custom_color_variables() {
		...
		wp_enqueue_script(
			...
			array( 'dark-mode-support-toggle' ),
			'1.0.0',
			true
		);
	}
	...
	public function enqueue_scripts() {
		...
		$url = get_template_directory_uri() . '/assets/scripts/css/style-dark-mode.css';
		if ( is_rtl() ) {
			$url = get_template_directory_uri() . '/assets/scripts/css/style-dark-mode-rtl.css';
		}
		wp_enqueue_style( 'tt1-dark-mode', $url, array( 'style' ), wp_get_theme()->get( 'Version' ) ); // @phpstan-ignore-line. Version is always a string.
	}
	...
	public function customize_controls_enqueue_scripts() {
		...
		wp_enqueue_script(
			...
			array( 'customize-base', 'customize-controls', 'underscore', 'jquery', 'customize-helpers' ),
			'1.0.0',
			true
		);
	}
	...
	public function customizer_controls( $wp_customize ) {
		...
		if ( is_object( $colors_section ) ) {
			$colors_section->title = __( 'Colors & Dark Mode', '__THEMENAE__' );
		}

		// Custom notice control.
		include_once get_theme_file_path( 'inc/classes/class-customize-notice-control.php' ); // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		...
		$description  = '<p>';
		$description .= sprintf(
			/* translators: %s: _THEMENAE_ support article URL. */
			__( 'Dark Mode is a device setting. If a visitor to your site requests it, your site will be shown with a dark background and light text. <a href="%s">Learn more about Dark Mode.</a>', '__THEMENAE__' ),
			esc_url( __( 'https://wordpress.org/support/article/_THEMENAE_/#dark-mode-support', '__THEMENAE__' ) )
		);
		$description .= '</p>';
		$description .= '<p>' . __( 'Dark Mode can also be turned on and off with a button that you can find in the bottom corner of the page.', '__THEMENAE__' ) . '</p>';
		...
		$wp_customize->add_control(
			...
			array(
				'type'            => 'checkbox',
				'section'         => 'colors',
				'label'           => esc_html__( 'Dark Mode support', '__THEMENAE__' ),
				'priority'        => 110,
	...
	public function the_html( $attrs = array() ) {
		...
		printf(
			...
			esc_html__( 'Dark Mode: %s', '__THEMENAE__' ),
		...
		<style>
			...
			#dark-mode-toggler > span::before {
				content: '<?php esc_attr_e( 'Off', '__THEMENAE__' ); ?>';
			}
			#dark-mode-toggler[aria-pressed="true"] > span::before {
				content: '<?php esc_attr_e( 'On', '__THEMENAE__' ); ?>';
			}
	...
	public function the_script() {
		echo '<script>';
		include get_template_directory() . '/assets/scripts/js/dark-mode-toggler.js'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude
		echo '</script>';
	}
	...
	public function add_privacy_policy_content() {
		...
		$content = '<p class="privacy-policy-tutorial">' . __( '_THEMENAE_ uses LocalStorage when Dark Mode support is enabled.', '__THEMENAE__' ) . '</p>'
				. '<strong class="privacy-policy-tutorial">' . __( 'Suggested text:', '__THEMENAE__' ) . '</strong> '
				. __( 'This website uses LocalStorage to save the setting when Dark Mode support is turned on or off.<br> LocalStorage is necessary for the setting to work and is only used when a user clicks on the Dark Mode button.<br> No data is saved in the database or transferred.', '__THEMENAE__' );
		wp_add_privacy_policy_content( '__THEMENAE__', wp_kses_post( wpautop( $content, false ) ) );
	}
