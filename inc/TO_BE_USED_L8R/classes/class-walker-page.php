...
if ( ! class_exists( 'Walker_Page' ) ) {
	...
	class Walker_Page extends Walker_Page {
		 ...
		 *
		 * @since _THEMENAE_ 1.0
		 * @since _THEMENAE_ 1.9 Renamed `$page` to `$data_object` and `$current_page` to `$current_object_id`
		...
		public function start_el( &$output, $data_object, $depth = 0, $args = array(), $current_object_id = 0 ) {
			...
			if ( '' === $page->post_title ) {
				...
				$page->post_title = sprintf( __( '#%d (no title)', '__THEMENAE__' ), $page->ID );
			}
			...
			if ( isset( $args['show_toggles'] ) && true === $args['show_toggles'] ) {
				...
				if ( isset( $args['pages_with_children'][ $page->ID ] ) ) {
					...
					$args['list_item_after'] .= '<button class="toggle sub-menu-toggle fill-children-current-color" data-toggle-target="' . $toggle_target_string . '" data-toggle-type="slidetoggle" data-toggle-duration="' . absint( $toggle_duration ) . '" aria-expanded="false"><span class="screen-reader-text">' . __( 'Show sub menu', '__THEMENAE__' ) . '</span>' . get_theme_svg( 'chevron-down' ) . '</button>';
