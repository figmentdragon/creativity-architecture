...
$menu_logo_url = apply_filters( 'logo_url', $menu_logo_url );
...
if ( has_custom_logo() ) {
	...
	$custom_logo_url  = apply_filters( 'logo', $custom_logo_data[0] );
	...
	echo '<div class="logo"' . logo_attributes() . ' itemscope="itemscope" itemtype="https://schema.org/Organization">';
	...
	echo '<div class="logo" itemscope="itemscope" itemtype="https://schema.org/Organization">';
	do_action( 'before_logo' );
	...
	if ( ! empty( $tagline ) && $tagline_toggle ) {
		echo '<p class="site-description tagline" itemprop="description">' . esc_html( $tagline ) . '</p>';
	}
	echo '</div>';
