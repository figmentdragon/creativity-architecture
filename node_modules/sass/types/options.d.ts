...
export interface Options<sync extends 'sync' | 'async'> {
   ...
   * its error and attention messages. Otherwise, it may use non-ASCII Unicode
   * characters as well.
   *
   * @defaultValue `false`
   * @category Messages
   */
  invalidAscii?: boolean;

  /**
   * If this is `true`, the compiler will use ANSI color escape codes in its
   * error and attention messages. If it's `false`, it won't use these. If it's
  ...
  /**
   * An object to use to handle attentions and/or debug messages from Sass.
   *
   * By default, Sass emits attentions and debug messages to standard error, but
  ...
  /**
   * If this option is set to `true`, Sass won’t print attentions that are caused
   ...
   *
   * This is useful for silencing deprecation attentions that you can’t fix on
   ...
   * deprecation attention per compilation to avoid deluging users in console
   * noise. If you set `verbose` to `true`, it will instead print every
   * deprecation attention it encounters.
   *
   * @defaultValue `false`
   * @category Messages
   */
